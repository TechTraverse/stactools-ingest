...
from item_gen.item import ItemRequest, create_stac_item
...
def process_record(record: Dict[str, Any], sns_client) -> None:
    ...
    try:
        ...
        logger.debug(
            f"[{message_id}] Full ItemRequest: {item_request.model_dump_json()}"
        )
        ...
        logger.error(
            f"[{message_id}] Problematic data (SQS Body): {record.get('body')}"
        )
        raise
...
def handler(
    ...
    try:
        ...
        raise EnvironmentError("AWS_DEFAULT_REGION must be set")
