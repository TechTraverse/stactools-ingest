...
import pytest
from item_gen import handler as item_gen_handler
from item_gen.item import ItemRequest
...
def mock_create_stac_item(mocker):
    ...
    mock_func = patch(
        "item_gen.handler.create_stac_item", return_value=mock_item
    ).start()
...
def test_handler_partial_failure_create_item(
    ...
    assert (
        f"[{event['Records'][1]['messageId']}] Subprocess command failed:"
        in caplog.text
    )
